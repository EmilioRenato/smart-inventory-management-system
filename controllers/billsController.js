...
export const addBillsController = async (req, res) => {
  try {
    ...
    if (!sellerCode) {
      return res.status(400).json({ message: 'sellerCode is required' });
    }

    const cleanCode = String(sellerCode).replace(/\D/g, '').slice(0, 5);
    if (cleanCode.length !== 5) {
      return res.status(400).json({
        message: 'El código del vendedor debe tener 5 dígitos',
      });
    }
    ...
    // cliente
    if (!customerCedula) {
      return res.status(400).json({ message: 'customerCedula is required' });
    }
    if (!customerName) {
    ...
    if (!customerAddress) {
      return res.status(400).json({ message: 'customerAddress is required' });
    }
    ...
    if (!Number.isFinite(paid) || paid <= 0) {
      return res.status(400).json({
        message: 'paidTotal is required and must be > 0',
      });
    }

    if (paid > suggestedTotal) {
      return res.status(400).json({
        message: 'El precio a pagar no puede ser mayor al precio sugerido',
      });
    }
